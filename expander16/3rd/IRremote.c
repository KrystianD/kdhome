...
long decodeNEC(struct decode_results *results) {
  ...
  // Initial mark
  if (!MATCH_MARK(results->rawbuf[offset], NEC_HDR_MARK)) { if (!MATCH_MARK(results->rawbuf[offset], 4500))
    return ERR;
  }
  offset++;
  // Check for repeat
...
long decodePanasonic(struct decode_results *results) {
    ...
    // decode address
    int i; for (i = 0; i < PANASONIC_BITS; i++) {
...
long decodeHash(struct decode_results *results) {
  ...
  int i; for (i = 1; i+2 < results->rawlen; i++) {
