...
//#include "ssc.h"
#include "../user/user_comm.h"

char buffer[1024];
volatile int rdPtr = 0, wrPtr = 0;
...
//  }
	if (UART_RXFIFO_FULL_INT_ST == (READ_PERI_REG(UART_INT_ST(uart_no)) & UART_RXFIFO_FULL_INT_ST))
	{
		...
		buffer[wrPtr] = temp;
		int newWrPtr = (wrPtr + 1) % sizeof(buffer);
		wrPtr = newWrPtr;
...
uart_process()
{
	while (rdPtr != wrPtr)
	{
		commProcessChar(buffer[rdPtr]);
		int newRdPtr = (rdPtr + 1) % sizeof(buffer);
		rdPtr = newRdPtr;
	}
}
